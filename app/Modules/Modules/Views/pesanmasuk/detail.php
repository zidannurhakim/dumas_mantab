    ...
    <div class="py-3 py-lg-4">
        <div class="row">
            <div class="col-lg-6">
                ...
                <a href='<?= base_url('module/pesan-masuk'); ?>' class="btn btn-success">Kembali</a>
            </div>
            <div class="col-lg-6">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12">
                            <?php if($data->data_status_selesai == "BELUM"){ ?>
                                <form id="selesaiData" enctype="multipart/form-data">
                                    <input type="hidden" name="<?= csrf_token(); ?>" value="<?= csrf_hash(); ?>">
                                    <input type="hidden" name="usr_id" value="<?= session()->usr_id; ?>">
                                    <div>
                                        <div class="mb-2 d-grid">
                                            <button class="btn btn-danger" id="submitSelesai">Tombol Penyelesaian</button>
                                        </div>
                                    </div>
                                </form>
                            <?php }elseif($data->data_status_selesai == "SELESAI"){ ?>
                                <div class="col-md-12 mb-4 d-grid">
                                    <button class="btn btn-sm btn-primary">Data Telah Diselesaikan</button>
                                </div>
                            <?php }else{ ?>
                                <div class="col-md-12 mb-4 d-grid">
                                    <button class="btn btn-sm btn-danger">Flag Tidak Diketahui</button>
...
<script type="text/javascript">
    ...
    function fetchChatMessages() {
        ...
        $.ajax({
            url: `<?= base_url('module/pesan-masuk/') ?>${currentDataId}/data-obrolan`,
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                ...
                sortedMessages.forEach(function(message) {
                    ...
                    var messageClass = (message.chat_jenis === 'PENGELOLA') ? 'message-sent' : 'message-received';
    ...
    $(document).ready(function() {
        ...
        $('#submitChat').click(function(e) {
            ...
            $.ajax({
                url: '<?= base_url('module/pesan-masuk/') ?>' + currentDataId + '/proses-kirim-chat',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
    ...
    $('#submitSelesai').click(function(e) {
        e.preventDefault(); 

        var form = $('#selesaiData')[0];
        var formData = new FormData(form); 
        Swal.fire({
            title: 'Apakah Anda yakin?',
            text: "Data ini akan diselesaikan!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('module/pesan-masuk/'.$data_id.'/proses-selesai') ?>',
                    type: 'POST',
                    data: formData,
                    processData: false, 
                    contentType: false, 
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 'success') {
                            Swal.fire({
                                ...
                                window.location.href = '<?= base_url('module/pesan-masuk'); ?>';
                            });
                        } else {
                            Swal.fire({
                                icon: 'warning',
