    ...
    <div class="py-3 py-lg-4">
        <div class="row">
            <div class="col-lg-6">
                ...
                <a href='<?= base_url('layanan/akses-admin'); ?>' class="btn btn-success">Kembali</a>
            </div>
            <div class="col-lg-6">
...
<script type="text/javascript">
    ...
    function loadData() {
        ...
        $('#tblid').DataTable({
            ...
            ajax: {
                type: "POST",
                url: "<?= base_url('layanan/akses-admin/module-ajax') ?>",
                data: function(d) {
                    d.usg = userGroupId;
                    d[csrfName] = csrfHash;
                },
    ...
    $('tbody').on('click', '.privmod_upd', function() {
        ...
        $.ajax({
            type: "POST",
            url: "<?= base_url('layanan/akses-admin/module/privmod-upd') ?>",
            data: {
                "ops": ops,
                "usg": usg,
                "lay_id": lay_id,
            ...
            success: function(msg) {
                if (msg == 'success') {
                    if (ops == 'add') {
                        $('#'+lay_id).data('ops', 'del');
                        $('#'+lay_id).closest('td').prev().html("<center><span class='badge bg-success'>True</span></center>");
                    } else {
                        $('#'+lay_id).data('ops', 'add');
                        $('#'+lay_id).closest('td').prev().html("<center><span class='badge bg-warning'>False</span></center>");
                    }
                } else {
                    $('#'+lay_id).html('error!');
                }
